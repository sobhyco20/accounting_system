...
def post_balance(self):
    ...
    # إنشاء حركة مخزنية فقط دون قيد محاسبي
...
def unpost_balance(self, request, pk):
    ...
    if not balance.is_posted:
        self.message_user(request, "الرصيد غير مرحل مسبقًا.", messages.WARNING)
    ...
    balance.save()
    self.message_user(request, "تم إلغاء ترحيل الرصيد بنجاح.", messages.SUCCESS)
    return redirect('admin:inventory_openingstockbalance_change', pk)
